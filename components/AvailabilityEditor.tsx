...
function parseTimeToHourMinute(timeStr: string): { hour: number; minute: number } {
...
export function AvailabilityEditor({
  ...
  /** Only ask "this day vs all future" when user has set recurrence (ripetizione !== "mai"). */
  ...
  const idsToDeleteAllFuture = useMemo(() => {
    ...
    return availabilityForDate
      .filter((row) => {
        ...
        return dateStr >= selectedDateStr && daySlotPatternKeys.has(getRecordPatternKey(row));
      })
      .map((r) => r.id)
      .filter((id): id is number => id != null);
  }, [availabilityForDate, daySlotPatternKeys, selectedDateStr]);
