...
export function DashboardUpcomingBookings() {
  ...
  return (
    <div className="flex-1 bg-background/5 rounded-lg p-4 overflow-hidden">
      <h2 className="text-lg font-semibold text-foreground mb-2">Prenotazioni imminenti</h2>
      ...
      {!isLoading && !isFetching && !error && highlightedBookings.length === 0 && (
        <Empty className="flex flex-1 h-[calc(100%-5rem)] flex-col items-center justify-center rounded-md border border-dashed border-blue-300/40 bg-blue-900/20  py-8 text-center">
          <EmptyHeader>
            <EmptyMedia>
      ...
      {!isLoading && !isFetching && !error && highlightedBookings.length > 0 && (
        <div className="bg-background rounded-md  pt-4">
